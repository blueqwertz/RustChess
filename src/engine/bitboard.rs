...
pub struct BitPos {
	...
	pub en_passant_white: BitBoard,
	pub en_passant_black: BitBoard,
	pub wp: BitBoard,
	pub wn: BitBoard,
	pub wb: BitBoard,
	pub wr: BitBoard,
...
impl BitPos {
	...
	pub fn empty () -> Self {
		Self {
			...
			en_passant_white: BitBoard::empty(),
			en_passant_black: BitBoard::empty(),
			wp: BitBoard::empty(),
			wn: BitBoard::empty(),
			wb: BitBoard::empty(),
			wr: BitBoard::empty(),
	...
	pub fn make_move(&mut self, bit_move: Move) {
		...
		match bit_move.color {
			Color::White => {
				...
				} else if bit_move.en_passant != 0 {
					&self.en_passant_white.set_bit(bit_move.en_passant);
				}
				match bit_move.kind {
					Kind::King => {
			...
			Color::Black => {
				...
				if bit_move.capture {
					...
					&self.en_passant_black.set_bit(bit_move.en_passant);
				}
				match bit_move.kind {
					Kind::King => {
		...
		match bit_move.color {
			Color::White => {
				...
				if bit_move.capture {
					...
					&self.en_passant_white.unset_bit(bit_move.en_passant);
				}
				match bit_move.kind {
					Kind::King => {
			...
			Color::Black => {
				...
				if bit_move.capture {
					...
					&self.en_passant_black.unset_bit(bit_move.en_passant);
				}
				match bit_move.kind {
					Kind::King => {
	...
	pub fn can_move_direction(&self, position: u8, direction: usize, color_to_move: Color) -> bool {
		let now = Instant::now();
		let mut check_board = BitBoard::empty();
		match color_to_move {
			Color::White => {
				for dir in 0usize..8usize {
					if dir == direction {continue}
					else {
						check_board.0 |= self.pinned_white[dir].0;
					}
				}
			}
			Color::Black => {
				for dir in 0usize..8usize {
					if dir == direction {continue}
						...
						check_board.0 |= self.pinned_black[dir].0;
					}
				}
			}
			Color::Undefined => {}
